...
class APFlowFreeWorld(World):
    ...
    # Classification comes from the item tables, so items created by name alone (start inventory,
    # plando, item links) still carry the right classification — a level unlock is always progression.
    item_name_to_classification = {name: data[1] for name, data in ITEMS.items()}
    item_name_to_classification.update({name: data[1] for name, data in ITEMS_B.items()})
    item_name_to_classification.update({name: data[1] for name, data in ITEMS_C.items()})
    item_name_to_classification.update({name: data[1] for name, data in ITEMS_FILLER.items()})
    item_name_to_classification.update({name: data[1] for name, data in ITEMS_TRAPS.items()})
    item_name_to_classification.update({name: data[1] for name, data in ITEMS_USEFUL.items()})

    def create_item(self, name: str) -> APFlowFreeItem:
        """
        ...
        return APFlowFreeItem(name, self.item_name_to_classification[name], self.item_name_to_id[name], self.player)

    def get_filler_item_name(self) -> str:
        # Used by item links / plando to pad the pool; the default could pick a level unlock.
        return "Flow Bonus"
    ...
    def create_items(self):
        ...
        itempool = [self.create_item(name) for name in selected_names]
    ...
    def fill_slot_data(self):
        ...
        # Generate a seed for each stage within each level. Uses the world's seeded RNG so the same
        # multiworld seed always produces the same puzzles.
        seeds = {}
        for level in range(1, num_levels + 1):
            level_name = f"Level {level}"
            # Generate a list of random seeds for the given number of stages.
            seeds[level_name] = [self.random.randint(100000000, 999999999) for _ in range(stages_per_level)]
